...
export function WelcomeChecklist({ hasProfile, hasPlan, loggedWeight, loggedWorkout, isPro }: Props) {
  const [dismissed, setDismissed] = useState<boolean>(() => {
    try { return typeof window !== "undefined" && localStorage.getItem("myfp:checklist_dismissed") === "1"; } catch { return false; }
  });
  ...
  return (
    <section className="surface-card p-4 mb-4 animate-fade-in">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">{done}/{total} complete · {pct}%</p>
          </div>
        </div>
        <button onClick={() => { try { localStorage.setItem("myfp:checklist_dismissed", "1"); } catch {}; setDismissed(true); }}
      ...
      <ul className="space-y-1.5">
        {steps.map((s) => (
          <li key={s.id}>
            <Link to={s.to as any} className={`flex items-center gap-2.5 p-2 rounded-xl text-sm transition ${s.done ? "text-muted-foreground line-through" : "hover:bg-muted/50"}`}>
              {s.done ? (
