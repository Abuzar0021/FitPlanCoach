...
function SettingsAdmin() {
  ...
  const [rules, setRules] = useState<CalorieRules>({
  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data }) => {
        ...
        if (get("social_links"))
          setSocial({ ...DEFAULT_SITE_CONFIG.social, ...(get("social_links") as any) });
        if (get("announcement"))
          setAnnouncement({ ...DEFAULT_SITE_CONFIG.announcement, ...(get("announcement") as any) });
        if (get("analytics"))
          setAnalytics({ ...DEFAULT_SITE_CONFIG.analytics, ...(get("analytics") as any) });
      });
  }, []);
  ...
  async function saveAll() {
    const rows = [
      ...
      {
        key: "analytics",
        value: { ga4_id: analytics.ga4_id.trim(), clarity_id: analytics.clarity_id.trim() } as any,
      },
    ];
    for (const r of rows) {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <section className="bg-card border border-border rounded-2xl p-5 space-y-3">
        ...
        <div className="space-y-1.5">
          ...
          <Input
            ...
            placeholder="New: weekly plan refresh is live 🎉"
          />
        </div>
        <div className="space-y-1.5">
          ...
          <Input
            ...
            onChange={(e) => setAnnouncement({ ...announcement, href: e.target.value })}
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) => setAnalytics({ ...analytics, ga4_id: e.target.value })}
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div>
            ...
            <Input
              inputMode="decimal"
              value={pricing.elite}
              onChange={(e) => setPricing({ ...pricing, elite: Number(e.target.value) })}
            />
          </div>
        </div>
      </section>
      ...
      <section className="bg-card border border-border rounded-2xl p-5 space-y-3">
        ...
        <div>
          <Label>Free plan generation limit</Label>
          <Input
      ...
      <section className="bg-card border border-border rounded-2xl p-5 space-y-3">
        ...
        <div>
          ...
          <Input
            inputMode="decimal"
            value={rules.protein_per_kg}
            onChange={(e) => setRules({ ...rules, protein_per_kg: Number(e.target.value) })}
          />
        </div>
        <div>
            ...
            {(["sedentary", "light", "moderate", "active"] as const).map((k) => (
              <div key={k}>
                ...
                <Input
                  ...
                  value={rules.tdee[k]}
                  onChange={(e) =>
                    setRules({ ...rules, tdee: { ...rules.tdee, [k]: Number(e.target.value) } })
                  }
                />
              </div>
            ))}
          ...
          <div className="grid grid-cols-3 gap-2">
            {(["lose_fat", "build_muscle", "maintain"] as const).map((k) => (
              <div key={k}>
                ...
                <Input
                  inputMode="decimal"
                  value={rules.goal_adjust[k]}
                  onChange={(e) =>
                    setRules({
                      ...rules,
                      goal_adjust: { ...rules.goal_adjust, [k]: Number(e.target.value) },
                    })
                  }
                />
              </div>
      ...
      <Button onClick={saveAll} className="h-11 px-6">
        Save all settings
      </Button>
    </div>
  );
