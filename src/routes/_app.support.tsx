...
function SupportPage() {
  ...
  const [form, setForm] = useState({
    subject: "",
    category: "technical" as (typeof CATEGORIES)[number]["v"],
    message: "",
  });
  ...
  const activeTicket = useMemo(
    () => tickets?.find((t) => t.id === active) ?? null,
    [tickets, active],
  );
        ...
        <div className="surface-card p-3 mb-4 flex items-center gap-3 animate-fade-in">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-muted-foreground">
              Your tickets are answered first — usually within one business day.
            </p>
          </div>
        </div>
      )}
        ...
        <form onSubmit={submitNew} className="surface-card p-4 mb-4 space-y-3 animate-fade-in">
          ...
          <div>
            ...
            <Textarea
              ...
              placeholder="Describe what happened, with steps if possible."
            />
          </div>
      ...
      {activeTicket && (
        <div className="surface-card p-4 mb-4 animate-fade-in">
          <button
            ...
            className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground hover:text-foreground mb-2"
          >
            ← Back
          </button>
          <div className="flex items-start justify-between gap-3 mb-1">
            ...
            <span
              className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full shrink-0 ${STATUS_COLORS[activeTicket.status]}`}
            >
              {activeTicket.status.replace("_", " ")}
            </span>
          </div>
          <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-3">
            {activeTicket.category} · #{activeTicket.id.slice(0, 8)}
          </p>
            ...
            {messages.map((m) => (
              ...
              >
                <p className="text-[10px] font-bold uppercase tracking-widest mb-1 text-muted-foreground">
                  {m.author_role === "staff" ? "Support" : "You"} ·{" "}
          ...
          {tickets === null ? (
            ...
            <div className="surface-card p-10 text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1 mb-5">
                Stuck on something? Open a ticket and we'll get back to you within{" "}
                {priority ? "one business day" : "1–2 business days"}.
              </p>
              <Button onClick={() => setCreating(true)}>
                <Plus className="size-4 mr-1.5" /> Open a ticket
              </Button>
              ...
              {tickets.map((t) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3 mb-1">
                    ...
                    <span
                      className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full shrink-0 ${STATUS_COLORS[t.status]}`}
                    >
                      {t.status.replace("_", " ")}
                    </span>
                  </div>
