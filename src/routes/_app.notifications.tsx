    ...
    <MobileShell>
      ...
      <div className="space-y-2.5">
        {items?.map((n) => {
          ...
          const body = (
            <div
              className={`surface-card p-4 flex items-start gap-3 transition-colors ${n.link ? "hover:border-border-strong" : ""} ${n.read_at ? "opacity-70" : "border-primary/30"}`}
            >
          ...
          return n.link ? (
            <Link
              key={n.id}
              to={n.link as any}
              onClick={() => !n.read_at && markFn({ data: { ids: [n.id] } }).then(load)}
            >
              {body}
            </Link>
          ) : (
