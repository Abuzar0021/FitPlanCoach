...
export const Route = createFileRoute("/cms/media")({
  head: () => ({ meta: [{ title: "Media — Website CMS" }] }),
      ...
      <CmsHeader
        title={
          <>
            Media{" "}
            {assets && (
              <span className="text-sm text-muted-foreground font-normal">({assets.length})</span>
            )}
          </>
        }
        subtitle="Images for featured photos and in-article content — shared with the article editor's image picker."
        actions={
      ...
      {assets === null ? (
        ...
        <div className="surface-card p-12 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Upload images to reuse across articles.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
