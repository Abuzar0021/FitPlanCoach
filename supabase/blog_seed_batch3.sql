-- Third batch of blog content — run ONCE in Supabase's SQL Editor.
-- Fills out categories that already exist but are thin/empty (cardio,
-- recipes, supplements, muscle-gain, weight-loss, health-tips, workouts,
-- home-fitness), rather than piling more onto categories that already have
-- content. Longer-form than batch 2 (roughly 1,000-1,400 words each).
-- Safe to re-run: categories use ON CONFLICT DO NOTHING, posts use
-- ON CONFLICT (slug) DO NOTHING.

INSERT INTO public.blog_categories (slug, name, description)
VALUES
  ('cardio', 'Cardio', 'Conditioning, endurance, and heart-health training.'),
  ('recipes', 'Recipes', 'Simple, high-protein meals that fit a real schedule.'),
  ('supplements', 'Supplements', 'What''s actually worth taking, and what isn''t.'),
  ('muscle-gain', 'Muscle Gain', 'Building size and strength, especially for hard responders.'),
  ('weight-loss', 'Weight Loss', 'Sustainable fat loss without the extremes.'),
  ('health-tips', 'Health Tips', 'Recovery, sleep, and the habits around training.'),
  ('workouts', 'Workouts', 'Program design and how to structure your training week.'),
  ('home-fitness', 'Home Fitness', 'Training well without a commercial gym.')
ON CONFLICT (slug) DO NOTHING;
...
BEGIN
  ...
  (
    ...
    'Cardio advice ranges from "none, just lift" to "an hour a day." Here''s what the actual evidence supports for fat loss, heart health, and performance.',
    'Cardio recommendations vary wildly depending on your goal. This guide breaks down realistic weekly cardio targets for general health, fat loss, and athletic performance, and explains why more isn''t always better.',
    $html$<p><strong>Most healthy adults need roughly 150 minutes of moderate cardio per week for general heart health</strong> — about 20-30 minutes, five days a week — with more added on top depending on whether your goal is fat loss or performance. Beyond that baseline, "how much cardio" stops being one universal number and starts depending entirely on what you're actually training for.</p>

<h2>The baseline: general health</h2>
<p>Major health bodies converge on a similar number: roughly 150 minutes of moderate-intensity cardio per week (or 75 minutes of vigorous-intensity), spread across most days. This is the floor for cardiovascular health — lower risk of heart disease, better blood pressure, better insulin sensitivity — regardless of whether you're trying to lose weight or build muscle. If you're doing zero cardio right now, getting to this baseline is the single highest-value change you can make, well before worrying about optimizing further.</p>
...
<p>Build up gradually if you're starting from very little activity — jumping straight to daily hard cardio is a common way to burn out or get injured in the first few weeks. Track your sessions the same way you'd track a lifting program; consistency over months is what actually produces the heart-health and fat-loss benefits, not any single "perfect" week.</p>$html$,
    'published',
    v_author_id,
    (SELECT id FROM public.blog_categories WHERE slug = 'cardio'),
    now() - interval '9 days',
    'How much cardio you actually need per week for heart health, fat loss, and performance — and why more isn''t always better.'
  ),
  (
    'high-protein-meals-under-15-minutes',
    '5 High-Protein Meals You Can Make in Under 15 Minutes',
    'You don''t need an hour in the kitchen to eat well. These five meals are fast, high in protein, and require almost no cooking skill.',
    'Five genuinely fast, high-protein meals for busy days — each takes 15 minutes or less, uses simple ingredients, and requires no advanced cooking technique.',
    $html$<p>The biggest barrier to eating well on a busy day usually isn't knowing what to eat — it's not having 45 minutes to cook it. These five meals are all genuinely fast (15 minutes or less, most much quicker), high in protein, and don't require any real cooking skill.</p>
...
<p>None of these require special equipment or advanced technique — a pan, a microwave, and a can opener cover all five. The real trick to eating well on a busy schedule isn't finding more elaborate recipes, it's keeping a short rotation of meals like these that you genuinely don't mind repeating. Log whichever ones you actually make in FitPlanCoach's food diary — over a few weeks you'll see clearly which of these fit your macros and appetite best, and can build the rest of your week around them.</p>$html$,
    'published',
    v_author_id,
    (SELECT id FROM public.blog_categories WHERE slug = 'recipes'),
    now() - interval '8 days',
    'Five fast, high-protein meals you can make in 15 minutes or less — simple ingredients, no advanced cooking skill required.'
  ),
  (
    'supplements-actually-worth-your-money',
    'Which Supplements Are Actually Worth Your Money?',
    'The supplement industry sells hundreds of products with thin evidence behind them. Here''s an honest breakdown of what''s actually backed by research.',
    'A research-based breakdown of which supplements have real evidence behind them (creatine, protein powder, caffeine, vitamin D) versus which are mostly marketing with weak evidence.',
    $html$<p>Walk into any supplement store and you'll see hundreds of products, most implying they're essential for results. In reality, the list of supplements with genuinely strong evidence behind them is short — and it's worth knowing which few actually matter before spending money on the rest.</p>

<h2>Worth it: creatine monohydrate</h2>
<p>Creatine is one of the most researched supplements in existence, with decades of evidence supporting improved strength, power output, and muscle gain when combined with resistance training. It's also inexpensive and has a strong long-term safety record at standard doses (typically 3-5g per day). If you take one supplement beyond a basic protein source, this is the one with the best evidence-to-cost ratio.</p>
...
<p>If you want a simple, evidence-based starting point rather than researching every product individually: a protein source (whole food or powder) to hit your daily target, creatine monohydrate (3-5g daily), caffeine before training if you tolerate it well, and vitamin D if a blood test shows you're low. That combination is backed by real evidence, costs relatively little, and covers the supplements genuinely worth prioritizing.</p>$html$,
    'published',
    v_author_id,
    (SELECT id FROM public.blog_categories WHERE slug = 'supplements'),
    now() - interval '7 days',
    'An honest, evidence-based breakdown of which supplements are actually worth taking — creatine, protein, caffeine, vitamin D — and which aren''t.'
  ),
  (
    'how-to-build-muscle-as-a-hardgainer',
    'How to Build Muscle as a Hardgainer: A Practical Guide',
    'If you''ve struggled to gain weight and muscle no matter how much you train, the problem is almost always food, not your genetics or your program.',
    'A practical guide for people who struggle to gain muscle despite consistent training — covering realistic calorie surpluses, why "eating more" is harder than it sounds, and training adjustments that help.',
    $html$<p>If you've trained consistently for months with barely any visible muscle gain, it's tempting to blame genetics or assume you need some special "hardgainer" program. In reality, the overwhelming majority of hardgainer cases come down to one thing: not eating enough, consistently, for long enough. The fix is almost always nutritional before it's a training-program problem.</p>
...
<p>Being a hardgainer isn't a life sentence — it's a nutrition and consistency challenge more than a training one. Track your actual intake honestly, build in an intentional surplus through practical, low-effort additions, keep training volume reasonable, and give it real time before concluding it isn't working.</p>$html$,
    'published',
    v_author_id,
    (SELECT id FROM public.blog_categories WHERE slug = 'muscle-gain'),
    now() - interval '6 days',
    'A practical guide to building muscle as a hardgainer — realistic calorie surpluses, easy ways to add calories, and training adjustments that actually help.'
  ),
  (
    'why-your-weight-loss-has-stalled',
    'Why Your Weight Loss Has Stalled (And What Actually Fixes It)',
    'Weight loss plateaus are almost always explained by one of a handful of common causes. Here''s how to diagnose yours instead of guessing.',
    'A diagnostic guide to weight loss plateaus — covering the most common real causes (calorie creep, water retention, metabolic adaptation, inconsistent tracking) and what actually fixes each one.',
...
<p>A genuinely healthy, sustainable rate of fat loss is roughly 0.5-1% of bodyweight per week. If you lost weight faster than that in your first few weeks (common, and partly water weight), a subsequent slower rate isn't actually a plateau — it's the normal, sustainable rate reasserting itself after an initial faster drop.</p>
...
<p>Log your actual intake and your weight trend consistently in FitPlanCoach — most "mystery" plateaus resolve once you can see the real data clearly instead of relying on how a single day's number or how the week "felt."</p>$html$,
    'published',
    v_author_id,
    (SELECT id FROM public.blog_categories WHERE slug = 'weight-loss'),
    now() - interval '5 days',
    'A diagnostic guide to weight loss plateaus — the real common causes (calorie creep, water retention, metabolic adaptation) and what actually fixes each one.'
  ),
  (
    'how-sleep-affects-training-and-recovery',
    'How Sleep Affects Your Training, Recovery, and Results',
    'Sleep is the most underrated variable in fitness. Here''s what happens to your training and recovery when you don''t get enough, and how to actually improve it.',
    'Sleep quality directly affects strength, recovery, fat loss, and injury risk. This guide covers the real mechanisms and practical steps to improve sleep without complicated routines.',
    $html$<p>Of all the variables that affect training results, sleep is probably the most underrated and the most commonly neglected. People will carefully track macros and follow a structured program while running on 5-6 hours of sleep, not realizing how much that alone is limiting their results.</p>
...
<p>Sleep deprivation shifts hunger hormones in a direction that increases appetite and cravings, particularly for high-calorie, low-nutrient foods — a real, measurable effect, not just "being tired makes you eat junk food" folk wisdom. Poor sleep during a fat loss phase also increases the proportion of weight lost as muscle rather than fat, compared to the same calorie deficit with adequate sleep. In other words: the same diet plan produces meaningfully worse body-composition results when sleep is consistently poor.</p>
...
<p>If your training and nutrition are dialed in but progress still feels slower than it should, sleep is one of the first places worth honestly auditing — not as an afterthought, but as a genuine, equal pillar alongside training and diet.</p>$html$,
    'published',
    v_author_id,
    (SELECT id FROM public.blog_categories WHERE slug = 'health-tips'),
    now() - interval '4 days',
    'How sleep affects strength, recovery, fat loss, and injury risk — and practical, realistic steps to actually improve it.'
  ),
  (
    'push-pull-legs-vs-full-body',
    'Push Pull Legs vs Full Body: Which Workout Split Is Right for You?',
    'Two of the most popular training structures work very differently. Here''s how to decide which one actually fits your schedule and experience level.',
    'A practical comparison of Push Pull Legs (PPL) and Full Body training splits — covering who each one suits best, based on training frequency, recovery, and schedule constraints.',
    $html$<p>Two of the most common ways to structure a training week are Push Pull Legs (PPL) and Full Body — and despite endless online debate about which is "better," the honest answer is that each fits a different situation well. The right choice depends more on your schedule and recovery than on which is theoretically optimal.</p>
...
<p>Neither split is objectively superior — the split you'll actually complete consistently, at a frequency that matches your real schedule, will outperform a "better" split you only manage inconsistently. Be honest about how many days you can genuinely commit to before picking a program, not how many days you'd ideally like to train in a perfect week.</p>$html$,
    'published',
    v_author_id,
    (SELECT id FROM public.blog_categories WHERE slug = 'workouts'),
    now() - interval '3 days',
    'Push Pull Legs vs Full Body training splits compared — which one actually fits your schedule, recovery, and experience level.'
  ),
  (
    'home-gym-equipment-worth-buying',
    'The Only Equipment Worth Buying for a Home Gym (And What to Skip)',
    'Home gym equipment ranges from genuinely essential to expensive clutter. Here''s a practical, budget-conscious priority order.',
    'A practical, prioritized guide to home gym equipment — what genuinely expands your training options first, what to skip, and how to build up gradually without overspending.',
    $html$<p>It's easy to overspend on a home gym by buying equipment in the wrong order — expensive machines before basic essentials, or novelty items before the tools that actually expand what you can train. Here's a practical priority order based on cost-to-benefit, not what looks impressive in a photo.</p>
...
<p>A genuinely capable home gym — adjustable dumbbells, a pull-up bar, and resistance bands — can be assembled for a modest budget and covers the large majority of a well-structured strength program. Everything past that tier is a real upgrade, but not a requirement to train effectively. Build up gradually as your training experience (and your sense of what you'll actually use consistently) grows, rather than buying everything at once.</p>$html$,
    'published',
    v_author_id,
    (SELECT id FROM public.blog_categories WHERE slug = 'home-fitness'),
    now() - interval '2 days',
    'A practical, prioritized guide to home gym equipment — what''s genuinely worth buying first, and what to skip.'
  )
  ON CONFLICT (slug) DO NOTHING;
END $$;
